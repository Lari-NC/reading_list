...
const SearchBar = ({ onSearch }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/book/genres")
      .then((res) => res.json())
  ...
  const handleSearch = () => {
    if (searchOption === "Genre" && selectedGenre) {
      onSearch({ searchText: selectedBook, searchOption });
    } else {
  ...
  const handleGenreClick = (genre) => {
    const name = genre.display || genre.title || genre.name || "";
    setSearchText(name);
  ...
  const filteredGenres =
    searchOption === "Genre" && searchText
      ? genres.filter((g) => g.toLowerCase().includes(searchText.toLowerCase()))
      : genres;
